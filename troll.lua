...
-- ====== СОЗДАНИЕ GUI (СТИЛЬ RUZ HUB) ======
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "TrollMenuUltraV9"
screenGui.Parent = game.CoreGui
screenGui.Enabled = true

-- ====== ПЛАВАЮЩАЯ КНОПКА (RUZ STYLE) ======
...
toggleButton.BorderColor3 = Color3.fromRGB(100, 100, 255)
...
game:GetService("RunService").Heartbeat:Connect(function()
    if toggleButton.Visible then
        toggleButton.BackgroundTransparency = 0.1 + math.sin(tick() * 2) * 0.05
        toggleButton.Size = UDim2.new(0, 55 + math.sin(tick() * 2) * 2, 0, 55 + math.sin(tick() * 2) * 2)
    end
end)

-- ====== ОСНОВНОЕ МЕНЮ (УМЕНЬШЕННОЕ) ======
...
mainFrame.BorderColor3 = Color3.fromRGB(80, 80, 200)
mainFrame.ClipsDescendants = true
mainFrame.Parent = screenGui

-- Углы (скругление через UICorner)
...
titleText.Text = "⚡ TROLL ULTRA V9"
titleText.TextColor3 = Color3.fromRGB(150, 150, 255)
...
funcCount.Position = UDim2.new(0.65, 0, 0, 0)
funcCount.BackgroundTransparency = 1
funcCount.Text = "🔥230+"
funcCount.TextColor3 = Color3.fromRGB(255, 200, 50)
funcCount.TextScaled = true
funcCount.Font = Enum.Font.GothamBold
funcCount.Parent = titleBar

-- ====== КНОПКИ УПРАВЛЕНИЯ (RUZ STYLE) ======
local collapseButton = Instance.new("ImageButton")
collapseButton.Size = UDim2.new(0, 30, 0, 30)
collapseButton.Position = UDim2.new(0.88, 0, 0.5, -15)
collapseButton.BackgroundColor3 = Color3.fromRGB(50, 50, 80)
collapseButton.BorderSizePixel = 0
collapseButton.Image = "rbxassetid://6031094742"
collapseButton.ImageColor3 = Color3.fromRGB(200, 200, 255)
...
closeButton.ImageColor3 = Color3.fromRGB(255, 200, 200)
...
scrollingFrame.ScrollBarImageColor3 = Color3.fromRGB(80, 80, 200)
...
local function toggleCollapse()
    ...
    if isCollapsed then
        mainFrame:TweenSize(UDim2.new(0, 380, 0, 40), "Out", "Quad", 0.3, true)
        mainFrame:TweenPosition(UDim2.new(0.5, -190, 0, 0), "Out", "Quad", 0.3, true)
...
-- ====== ФУНКЦИЯ СОЗДАНИЯ КНОПОК (RUZ STYLE) ======
...
-- ====== ФУНКЦИЯ СОЗДАНИЯ КАТЕГОРИЙ (RUZ STYLE) ======
local function createCategory(text)
    local cat = Instance.new("Frame")
    cat.Size = UDim2.new(1, -6, 0, 28)
...
local function createPart(props)
    ...
    part.BrickColor = props.BrickColor or BrickColor.new("Bright red")
    part.Material = props.Material or Enum.Material.SmoothPlastic
    part.Transparency = props.Transparency or 0
    part.Anchored = props.Anchored or true
...
createButton("👤 К СЛУЧАЙНОМУ", Color3.fromRGB(200, 50, 50), function()
...
createButton("👥 К БЛИЖАЙШЕМУ", Color3.fromRGB(180, 30, 70), function()
    local closest = nil
...
createButton("🌌 В НЕБО", Color3.fromRGB(0, 100, 200), function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(0, 1000, 0)
end)

createButton("🌊 ПОД ВОДУ", Color3.fromRGB(0, 50, 150), function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(0, -100, 0)
end)

createButton("🏠 В ЦЕНТР", Color3.fromRGB(0, 150, 100), function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(0, 50, 0)
end)

createButton("🎯 РАНДОМ", Color3.fromRGB(150, 50, 200), function()
...
-- ====== DROPKICK ======
createCategory("💥 DROPKICK V2")

createButton("🦵 СУПЕР УДАР", Color3.fromRGB(255, 50, 0), function()
...
createButton("🚀 В НЕБО УДАР", Color3.fromRGB(0, 150, 255), function()
    local char = game.Players.LocalPlayer.Character
    local hrp = char.HumanoidRootPart
    for _, v in pairs(workspace:GetChildren()) do
        if v:IsA("Model") and v:FindFirstChild("Humanoid") and v ~= char then
...
createButton("🌀 КРУГОВОЙ УДАР", Color3.fromRGB(200, 0, 200), function()
    local char = game.Players.LocalPlayer.Character
    local hrp = char.HumanoidRootPart
    for i = 1, 36 do
...
createButton("🔥 ОГНЕННЫЙ УДАР", Color3.fromRGB(255, 100, 0), function()
    local char = game.Players.LocalPlayer.Character
    local hrp = char.HumanoidRootPart
    for i = 1, 20 do
...
createButton("🔥 ВЗРЫВНАЯ ВОЛНА", Color3.fromRGB(255, 200, 0), function()
    for i = 1, 10 do
        local exp = Instance.new("Explosion")
        exp.Position = game.Players.LocalPlayer.Character.HumanoidRootPart.Position + Vector3.new(0, i * 2, 0)
        exp.BlastRadius = 10 + i * 2
...
createButton("💫 КОСМИЧЕСКИЙ ВЗРЫВ", Color3.fromRGB(200, 0, 255), function()
    for i = 1, 15 do
        local exp = Instance.new("Explosion")
        local angle = (i / 15) * math.pi * 2
        exp.Position = game.Players.LocalPlayer.Character.HumanoidRootPart.Position + 
                       Vector3.new(math.cos(angle) * 20, math.random(-5, 15), math.sin(angle) * 20)
        exp.BlastRadius = 8
        exp.BlastPressure = 40000
        exp.Parent = workspace
        wait(0.05)
    end
end)
...
createButton("🌈 РАДУГА", Color3.fromRGB(255, 0, 255), function()
    local char = game.Players.LocalPlayer.Character
    for i = 1, 50 do
        local part = Instance.new("Part")
        part.Size = Vector3.new(1, 1, 1)
        part.Position = char.HumanoidRootPart.Position + Vector3.new(math.random(-30, 30), math.random(-30, 30), math.random(-30, 30))
...
createButton("🎆 САЛЮТ", Color3.fromRGB(255, 200, 0), function()
    ...
    for i = 1, 30 do
        ...
        local pos = char.HumanoidRootPart.Position + Vector3.new(math.random(-40, 40), math.random(5, 30), math.random(-40, 40))
        part.Position = pos
        part.BrickColor = BrickColor.Random()
        part.Material = Enum.Material.Neon
...
createButton("🌟 МАГИЧЕСКИЙ ШАР", Color3.fromRGB(255, 0, 200), function()
    ...
    ball.Position = char.HumanoidRootPart.Position + Vector3.new(0, 5, 0)
...
createButton("🌌 ГАЛАКТИКА", Color3.fromRGB(150, 0, 255), function()
    ...
    for i = 1, 80 do
        ...
        local angle = i * 0.5
        local radius = 8 + i * 0.3
        star.Position = char.HumanoidRootPart.Position + Vector3.new(math.cos(angle) * radius, math.sin(angle * 0.5) * 10, math.sin(angle) * radius)
...
createButton("✨ ИСКРЫ", Color3.fromRGB(255, 255, 200), function()
    ...
    for i = 1, 50 do
        ...
        spark.Position = char.HumanoidRootPart.Position + Vector3.new(math.random(-15, 15), math.random(-2, 8), math.random(-15, 15))
        spark.BrickColor = BrickColor.new("White")
        spark.Material = Enum.Material.Neon
        spark.Shape = Enum.PartType.Ball
        spark.Anchored = false
        spark.Parent = workspace
        spark.Velocity = Vector3.new(math.random(-30, 30), math.random(10, 40), math.random(-30, 30))
        wait(0.02)
    end
end)

createButton("💎 АЛМАЗНЫЙ ВЗРЫВ", Color3.fromRGB(0, 200, 255), function()
    local char = game.Players.LocalPlayer.Character
    for i = 1, 40 do
        local diamond = Instance.new("Part")
        diamond.Size = Vector3.new(1, 1, 1)
        local pos = char.HumanoidRootPart.Position + Vector3.new(math.random(-30, 30), math.random(-10, 20), math.random(-30, 30))
        diamond.Position = pos
        diamond.BrickColor = BrickColor.new("Bright blue")
        diamond.Material = Enum.Material.DiamondPlate
...
createButton("❄️ СНЕГОПАД", Color3.fromRGB(200, 200, 255), function()
    for i = 1, 100 do
        ...
        snow.Position = game.Players.LocalPlayer.Character.HumanoidRootPart.Position + 
                        Vector3.new(math.random(-50, 50), math.random(10, 40), math.random(-50, 50))
...
createButton("🌧️ ДОЖДЬ", Color3.fromRGB(0, 0, 200), function()
    for i = 1, 200 do
        ...
        rain.Position = game.Players.LocalPlayer.Character.HumanoidRootPart.Position + 
                        Vector3.new(math.random(-60, 60), math.random(10, 50), math.random(-60, 60))
        ...
        rain.Velocity = Vector3.new(0, -30, 0)
        wait(0.01)
    end
end)

createButton("🌋 ИЗВЕРЖЕНИЕ", Color3.fromRGB(255, 100, 0), function()
    local char = game.Players.LocalPlayer.Character
    for i = 1, 30 do
        local lava = Instance.new("Part")
        lava.Size = Vector3.new(3, 3, 3)
        lava.Position = char.HumanoidRootPart.Position + Vector3.new(math.random(-20, 20), i * 2, math.random(-20, 20))
...
createButton("💩 ДОЖДЬ КАКАШЕК", Color3.fromRGB(100, 50, 0), function()
    for i = 1, 50 do
...
createButton("💬 СПАМ В ЧАТ", Color3.fromRGB(200, 0, 200), function()
    local msgs = {"🔥 ТРОЛЛЬ АТАКУЕТ!", "💀 ВСЕ ВИДЯТ!", "😈 Я ТУТ!", "🤡 БУУУ!", "👻 ПРИВЕТ ВСЕМ!"}
    for i = 1, 30 do
...
createButton("🔄 ПЕРЕВЕРНУТЬ", Color3.fromRGB(150, 0, 150), function()
    local players = game.Players:GetPlayers()
    for _, v in pairs(players) do
        if v ~= game.Players.LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
...
createButton("📐 НОРМАЛЬНЫЙ", Color3.fromRGB(50, 150, 50), function()
...
-- ====== КЛОНЫ ======
createCategory("🧬 КЛОНЫ")

createButton("👥 10 КЛОНОВ", Color3.fromRGB(0, 200, 255), function()
    local char = game.Players.LocalPlayer.Character
    for i = 1, 10 do
...
createButton("🤖 50 КЛОНОВ", Color3.fromRGB(255, 0, 0), function()
    local char = game.Players.LocalPlayer.Character
    for i = 1, 50 do
        local clone = char:Clone()
        clone.Name = "Army_" .. i
...
createButton("🚗 МАШИНА", Color3.fromRGB(200, 0, 0), function()
    local char = game.Players.LocalPlayer.Character
    local car = Instance.new("Part")
...
createButton("🛸 НЛО", Color3.fromRGB(150, 0, 255), function()
    local char = game.Players.LocalPlayer.Character
...
createButton("✈️ САМОЛЁТ", Color3.fromRGB(0, 100, 200), function()
    local char = game.Players.LocalPlayer.Character
...
createButton("🐢 МЕДЛЕННО", Color3.fromRGB(0, 100, 0), function()
...
createButton("💫 ВЗОРВАТЬ ЧАСТИ", Color3.fromRGB(255, 100, 0), function()
    for _, v in pairs(workspace:GetChildren()) do
...
createButton("🗑️ УДАЛИТЬ ВСЁ", Color3.fromRGB(100, 0, 0), function()
    for _, v in pairs(workspace:GetChildren()) do
...
-- КАТЕГОРИЯ: ТЕКСТ
createCategory("💬 ТЕКСТ")

createButton("📝 БОЛЬШОЙ ТЕКСТ", Color3.fromRGB(255, 0, 0), function()
...
createButton("📢 РУПОР", Color3.fromRGB(255, 200, 0), function()
    local sound = Instance.new("Sound")
    sound.SoundId = "rbxassetid://9120407644"
    sound.Volume = 10
    sound.PlayOnRemove = true
...
-- ====== ВЫВОД В КОНСОЛЬ ======
print("🔥 ============================================ 🔥")
print("🔥     TROLL MENU ULTRA V9 ЗАГРУЖЕН!           🔥")
print("🔥 ============================================ 🔥")
print("🎨 СТИЛЬ RUZ HUB - УМЕНЬШЕННОЕ МЕНЮ!")
print("👀 ВСЕ ЭФФЕКТЫ ВИДНЫ ВСЕМ ИГРОКАМ!")
print("📌 INSERT - скрыть/показать меню")
print("📌 F1 - свернуть/развернуть меню")
print("📌 F2 - отключить/включить GUI")
print("🔥 ВСЕГО ФУНКЦИЙ: 230+!")
print("🔥 ============================================ 🔥")
